...
- 图片支持一次选择最多 8 张：DeepSeek 视觉模式会把连续截图作为同一个课表理解星期列、节次行、跨行色块与跨图衔接；本地 OCR 模式完全免费且不上传图片。
...
- DeepSeek API 仅在用户主动选择“视觉版面识别”或“校对结构”时调用。视觉模式使用 `deepseek-v4-flash-vision-exp`；402 会明确提示账户余额不足，并自动回退到本地 OCR，识别页同时提供阶段进度、耗时和取消按钮。
...
PDF、OCR、Excel 和 Word 解析器均采用动态导入，只在用户选择相应文件时加载，不进入日常查看课表的首屏执行路径；视觉模型通过 API 调用，不向安装包增加本地模型体积。
...
该命令依次执行单元测试、Android UI 静态适配守卫和生产构建。测试覆盖周次解析、跨周修改、课程时段分组、手势速度投影、夜间课程识别、实验课拆分、旧数据迁移、损坏数据恢复和提醒时间；适配守卫会检查 WebView 文字缩放、国产系统字体回退、卡片截断、系统栏同步及 Android 15 边到边行为。
...
- PDF、表格、文档与本地 OCR 模式均在设备端解析，不会默认上传。
- 只有用户填写 DeepSeek API Key 并主动选择“视觉版面识别”时，本次所选图片才会发送至 DeepSeek；主动点击“校对结构”时只发送识别文本。界面会在调用前明确提示。
