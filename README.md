...
| 管理学期 | 新建、切换、左滑删除；可设置第一周周一和 1–30 周学期长度 |
...
**DeepSeek 精确识别**是可选项，需要用户自己的 API Key。它主要用于版面复杂的截图、连续长图和本地结果校对；只有主动选择此模式时才会调用网络模型。
...
学期之间的数据彼此独立。导入时可以合并到已有学期，也可以建立新学期；在学期选择面板左滑某一项即可显示删除按钮，确认后才会真正删除。
...
- 免费本地解析不会上传课表文件。
- 只有填写 API Key 并主动选择 DeepSeek 精确识别或校对时，相关图片或本地提取的文本才会发送给模型服务。
